...
def init_app(app):
    ...
    def build_identity():
        ...
        # 1. I2P — строго по заголовкам, кука игнорируется
        if raw_i2p:
            anon_id = hashlib.sha256(f"{raw_i2p}{secret}".encode()).hexdigest()[:16]
            g.identity = {"id": anon_id, "transport": "i2p"}
            return

        # 2. Tor — определяем по onion-хосту, используем куку только здесь
        if host.endswith(".onion") or (".onion:" in host):
            ...
            if cookie_id:
                ...
                g.tor_set_cookie = new_id
            g.trust_score_override = 0
            return

        # 3. Клирнет — кука игнорируется
